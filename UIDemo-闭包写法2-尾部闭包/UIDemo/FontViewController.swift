...
class FontViewController : UIViewController {
    ...
    func setCallback4(name:String?, cb: ( controller:FontViewController,
        fontSize:Int, withColor:UIColor )->(Bool) ) {
        saveCb4 = cb;
    }
    
    // 不是尾部闭包
    func setCallback5(cb: ( controller:FontViewController,
        fontSize:Int, withColor:UIColor )->(Bool) , name:NSString?) {
        saveCb4 = cb;
    }
